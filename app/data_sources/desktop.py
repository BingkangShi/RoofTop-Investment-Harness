...
def probe_desktop_sources() -> list[dict]:
    qmt_home = Path(os.environ["ROOFTOP_QMT_HOME"]) if os.environ.get("ROOFTOP_QMT_HOME") else None
    tdx_home = Path(os.environ["ROOFTOP_TDX_HOME"]) if os.environ.get("ROOFTOP_TDX_HOME") else None
    ...
    return [
        {"code": "qmt", "ready": qmt_module, "mode": "xtquant read-only",
         "detail": "已发现 xtquant" if qmt_module else "设置 ROOFTOP_QMT_HOME 并启动券商 QMT/miniQMT"},
        {"code": "futu", "ready": _tcp_open("127.0.0.1", int(os.environ.get("ROOFTOP_FUTU_PORT", "11111"))),
         "mode": "OpenD local gateway", "detail": "检测本机 OpenD 端口；仍需核验对应市场行情权限"},
        {"code": "tdx_local", "ready": tdx_files > 0, "mode": "local .day files",
         "detail": f"发现 {tdx_files} 个日线文件" if tdx_files else "设置 ROOFTOP_TDX_HOME，并在通达信执行盘后数据下载"},
