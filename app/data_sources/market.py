...
DEFAULT_SYMBOLS = ("000001.SH", "510300", "159915", "600519")
USER_AGENT = "Mozilla/5.0 (Windows NT 10.0; Win64; x64) RoofTopMarketData/0.2"
HISTORY_INTERVALS = (5, 15, 1, 30, 60)
HISTORY_DAYS = 366 * 3
    ...
    with closing(connect()) as conn:
        ...
        for item in items:
            ...
            conn.execute(
                ...
                (item["price"], item["price"], item["symbol"]),
            )
...
def _mark_failure(source: str, dataset: str, symbol: str, started: str, exc: Exception) -> None:
    ensure_data_lake()
    dead = DATA_LAKE / "dead_letter" / f"{source}_{dataset}_{symbol.replace('.', '_')}_{datetime.now():%Y%m%dT%H%M%S}.json"
