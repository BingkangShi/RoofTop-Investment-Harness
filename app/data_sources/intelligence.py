...
contact identity in ROOFTOP_SEC_IDENTITY.
"""
...
def source_access_status() -> list[dict]:
    return [
        {"code": "bilibili", "configured": os.environ.get("ROOFTOP_BILIBILI_BROWSER", "").lower() in {"chrome", "edge", "firefox"},
         "cost": "免费", "auth": "可选：你本人在本机浏览器登录；无需把账号交给系统",
         "mode": "yt-dlp 公开元数据；可选复用本机浏览器 Cookie", "writes": False},
        {"code": "x_official", "configured": bool(os.environ.get("ROOFTOP_X_BEARER_TOKEN")),
         "cost": "官方读接口按量付费", "auth": "开发者项目 Bearer Token，仅写本机环境变量",
         "mode": "官方 X API v2；零付费约束下保持禁用", "writes": False},
        {"code": "sec_edgar", "configured": bool(os.environ.get("ROOFTOP_SEC_IDENTITY")),
...
def collect_bilibili(target: str, use_browser: bool = False) -> dict:
    ...
    browser = os.environ.get("ROOFTOP_BILIBILI_BROWSER", "chrome").lower()
    if use_browser:
        if browser not in {"chrome", "edge", "firefox"}:
            raise ValueError("ROOFTOP_BILIBILI_BROWSER 只允许 chrome/edge/firefox")
...
def collect_x_recent(query: str, max_results: int = 10) -> dict:
    token = os.environ.get("ROOFTOP_X_BEARER_TOKEN")
    if not token:
        raise RuntimeError("未配置 ROOFTOP_X_BEARER_TOKEN；X 账号本身不等于 API 权限，官方读接口按量付费")
    ...
    request = Request(url, headers={"Authorization": f"Bearer {token}", "User-Agent": "RoofTopLocalResearch/0.1"})
...
def collect_sec_submissions(cik: str) -> dict:
    identity = os.environ.get("ROOFTOP_SEC_IDENTITY", "").strip()
    if "@" not in identity:
        raise RuntimeError("请在本机设置 ROOFTOP_SEC_IDENTITY，例如 'RoofTopResearch your-email@example.com'")
