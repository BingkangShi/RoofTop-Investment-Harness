...
def _minute_source_rows(conn, symbol: str, interval_minutes: int = 1) -> list[dict]:
    rows = conn.execute(
        """SELECT m.bar_time,m.open,m.high,m.low,m.close,m.volume,m.amount,m.bar_kind,ds.code AS source
           FROM minute_bars m JOIN data_sources ds ON ds.id=m.source_id
           WHERE m.asset_symbol=? AND m.interval_minutes=? AND m.source_id=(
             SELECT source_id FROM minute_bars WHERE asset_symbol=? AND interval_minutes=?
             GROUP BY source_id
             ORDER BY (julianday(MAX(bar_time))-julianday(MIN(bar_time))) DESC,
                      COUNT(*) DESC,MAX(captured_at) DESC LIMIT 1)
...
def _daily_source_rows(conn, symbol: str) -> list[dict]:
    rows = conn.execute(
        """SELECT m.trade_date,m.open,m.high,m.low,m.close,m.volume,m.amount,ds.code AS source
           ...
           WHERE m.asset_symbol=? AND m.adjust_mode='qfq' AND m.source_id=(
             SELECT source_id FROM market_daily_bars WHERE asset_symbol=? AND adjust_mode='qfq'
             GROUP BY source_id ORDER BY (julianday(MAX(trade_date))-julianday(MIN(trade_date))) DESC,
             COUNT(*) DESC,MAX(captured_at) DESC LIMIT 1) ORDER BY m.trade_date""", (symbol, symbol),
    ).fetchall()
    if not rows:
        rows = conn.execute(
