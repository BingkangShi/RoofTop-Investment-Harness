...
不要在聊天、Issue、截图或 `configure_list.json` 中发送/填写以下内容：平台密码、短信验证码、Cookie、`auth_token`、`ct0`、Bearer Token、浏览器 Profile 目录。`configure_list.json` 只用于模型路由，不用于社交平台凭证。
...
3. 在启动 `run.ps1` 的同一个 PowerShell 窗口设置浏览器类型，例如 `$env:ROOFTOP_BILIBILI_BROWSER='chrome'`；
...
```powershell
$env:ROOFTOP_X_BEARER_TOKEN='<your API KEY>'
.\run.ps1
```
...
```powershell
$env:ROOFTOP_SEC_IDENTITY='RoofTopResearch your-email@example.com'
.\run.ps1
```
